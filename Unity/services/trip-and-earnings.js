 ...
 * related to trips and rider earnings from completed trips.
 * 
...
const processUnity = (command) => new Promise((resolve, reject) => {
    ...
    mutex.acquire().then(async (release) => {
        try {
            switch (command.cmd) {
                case 'complete-trip':
                    return resolve(await completeTrip(command.p));
                case 'record-transaction':
                    return resolve(await recordTransaction(command.p));
            }
        } catch (err) {
            console.error(err);
            reject(err);
...
const completeTrip = (x) => new Promise(async (resolve, reject) => {
    let {tid, stage, subStage} = x;

    let trip = await api.fetchOne({
        db: 'mongo',
        table: 'Trips',
        condition: {
            tid: tid
    ...
    //internally call recordTransaction function
    reject('ER500 - Not yet implemented');
});

const reopenTrip = (x) => new Promise(async (resolve, reject) => {
    let {tid, stage, subStage} = x;
    ...
    //internally call recordTransaction function. This should be exactly reverse of the complete trip transaction
    reject('ER500 - Not yet implemented');
});

const recordTransaction = (x) => new Promise(async (resolve, reject) => {
    const {rid, debit, credit, description, tid} = x;
    if(rid == null || rid == '') return reject('ER704,rid');
    if(debit == null || debit == '' || !(typeof debit === 'number')) return reject('ER704,debit');
    if(credit == null || credit == '' || !(typeof credit === 'number')) return reject('ER704,credit');
    if(description == null || description == '') return reject('ER704,description');

    let balance = await fetchBalance(rid);
    balance = balance - x.debit + x.credit;

    let transaction = {
        txid: utils.makeid(36),
        debit: x.debit,
        credit: x.credit,
...
const fetchBalance = (rid) => new Promise(async(resolve, reject) => {
    let transactions = await api.post(URLS.SCM_DB_FETCH, {
        db: 'mongo',
        table: 'TripEarnings',
        condition: {
            rid: rid
    ...
    if(transactions == null) return reject('ER500');
    if(transactions.length == 0) return resolve(0.0);

    transactions.sort((a,b) => a.timestamp - b.timestamp);

    transactions.forEach(x => {
        console.log(`${x.timestamp} | ${x.debit} | ${x.credit} | ${balance}`);
    })

    resolve(transactions[0].balance);
})
